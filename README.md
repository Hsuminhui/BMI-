# BMI計算機
專案簡介 : 此專案為BMI計算機，能夠自動計算出BMI值以及理想體重。

執行說明:

1. 在"輸入"的區塊中，有能夠輸入身高(cm)、體重(kg)的欄位、"計算"及"清除"的按鈕。

2. 當輸入的格式為非數字，將會有錯誤視窗彈出。(如下)
...
1.無滑鼠操作。能夠使用tab及enter來依序輸入測試值、進行測試。

2.當輸入值有誤時(eg:缺少輸入值、輸入值為非數值)，輸出欄位會一併清除。
